...
import (
	...
	routerip "github.com/Inspur-Data/router/pkg/ip"
...
func (r *IPPool) validCIDR(ctx context.Context) *field.Error {
	//checkout CIDR
	if err := routerip.IsCIDR(*r.Spec.IPVersion, r.Spec.CIDR); err != nil {
		return field.Invalid(
			cidrField,
			r.Spec.CIDR,
			err.Error(),
	...
	for _, pool := range poollist.Items {
		if *pool.Spec.IPVersion == *r.Spec.IPVersion {
			...
			overlap, err := routerip.IsCIDROverlap(*r.Spec.IPVersion, r.Spec.CIDR, pool.Spec.CIDR)
			if err != nil {
...
func (r *IPPool) isIPRangeOverlap(pool *IPPool) bool {
	for _, iprange := range r.Spec.IPs {
		for _, subIpRange := range pool.Spec.IPs {
			if overlaped, _ := routerip.IsIPRangeOverlap(*r.Spec.IPVersion, iprange, subIpRange); overlaped {
				return true
			}
		}
	}
...
func (r *IPPool) validAvailableIPs(ctx context.Context) *field.Error {
	...
	for _, excludeIP := range r.Spec.ExcludeIPs {
		valid, err := routerip.ContainsIP(*r.Spec.IPVersion, r.Spec.CIDR, excludeIP)
		if err != nil {
